...
class FrontendMenuBuilder extends MenuBuilder
{
    ...
    public function createMainMenu(Request $request)
    {
        ...
        if ($this->securityContext->getToken() && $this->securityContext->isGranted('ROLE_USER')) {
            ...
            $menu->addChild('register', array(
                ...
                'labelAttributes' => array('icon' => 'icon-user icon-large', 'iconOnly' => false,'text' => $this->translate('sylius.frontend.menu.main.before_register'))
    ...
    public function createCartMenu(Request $request)
    {
        ...
        $menu->addChild('cart', array(
            ...
            'labelAttributes' => array('icon' => 'icon-shopping-cart icon-large',
            							'cart_item_count'=> $this->translate('sylius.frontend.menu.main.cart_item_count', array(
	...
	/**
     * Builds frontend promotions menu.
     *
     * @param Request $request
     *
     * @return ItemInterface
     ...
     * @return ItemInterface
     */
    public function createTaxonomiesSidebarMenu(Request $request)
    {
    	
		$route_current = $request->attributes->get('_route');
    	
        $menu = $this->factory->createItem('Rayons', array(
            'childrenAttributes' => array(
                'class' => 'box-category list'
            )
        ));

        $childOptions = array(
            'childrenAttributes' => array('class' => 'nav-list list'),
            'labelAttributes'    => array('class' => 'nav-header'),
        );
        ...
        foreach ($taxonomies as $taxonomy) {
            ...
            $this->createTaxonomiesSidebarMenuNode($child, $taxonomy->getRoot(), $route_current);
        }

        return $menu;
    }

    private function createTaxonomiesSidebarMenuNode(ItemInterface $menu, TaxonInterface $taxon, $route_current)
    {
    	//$nb_products = $taxon->getProducts()->count();
    	
    	
        foreach ($taxon->getChildren() as $child) {
        	$nb_products = $child->getProducts()->count();
	        if($child->getChildren()->count() > 0){
	    		foreach ($child->getChildren() as $enfant) {
	    			$nb_products = $nb_products + $enfant->getProducts()->count();
	    		}
	    	}
        	
        	//var_dump($route_current == $child->getPermalink());
        	if($route_current == $child->getPermalink()){
        		$childMenu = $menu->addChild($child->getName(), array(
                'route'           => $child,
                'labelAttributes' => array('icon' => 'icon-angle-right')
            	))->setCurrent(true)->setExtras(['nb_products' => $nb_products ]);
        	}else{
        		$childMenu = $menu->addChild($child->getName(), array(
                'route'           => $child,
                'labelAttributes' => array('icon' => 'icon-angle-right')
            	))->setCurrent(false)->setExtras(['nb_products' => $nb_products ]);
        	}
            ...
            $this->createTaxonomiesSidebarMenuNode($childMenu, $child, $route_current);
        }
    }
